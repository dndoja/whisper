...
mixin ChaseMovement on Movement {
  ...
  void chaseTarget(
    ...
    _target = target;
    _onFinish = onFinish;
    calculatePathTo(target.absoluteCenter);
  }

  void calculatePathTo(Vector2 target) {
    _currentIndex = 0;
    _removeLinePathComponent();
    _currentPathTarget = target.clone();
    _currentPath = getPathToPosition(target);
  ...
  void update(double dt) {
    ...
    final targetLoc = _target?.absoluteCenter;
  ...
  List<Vector2> _calculatePath(Vector2 finalPosition) {
    ...
    if (_barriers.contains(targetPosition)) {
      _barriers.remove(targetPosition);
    }
